...
uint8_t buffer[16];           // Store values read from ADC
...
void read_dma_adc_1() {
  ...
  uint16_t adc_value[bytes_read / 2];
  //float adc_value_1[bytes_read / 2];

  if (ret == ESP_OK) {
    for (int i = 0; i < bytes_read / 2; i++) {
      adc_digi_output_data_t *p = (adc_digi_output_data_t *)&buffer[i];
      //adc_value[i] = p->type1.data;
      Serial.println(i);
    }
  }
  // for (int i = 1; i < (bytes_read / 2) - 1; i++) {
  //adc_value_1[i] = (adc_value[i - 1] + adc_value[i + 1]);
  //  Serial.println(i);
  //}
}
...
void setup() {
  ...
  adc_pattern[0].channel = ADC_CHANNEL_7;      // GPIO36 -> ADC1_CH7
  ...
  adc_continuous_config_t adc_chan_config = {
    ...
    .sample_freq_hz = 512000     // Samples per second
  };
...
*/
    ...
    Serial.print(";");
    Serial.println(avg_adc - avg_calib);
    */
  }
}